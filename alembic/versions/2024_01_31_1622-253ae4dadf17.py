"""Implement handing fee related tables

Revision ID: 253ae4dadf17
Revises: 028938fda4a9
Create Date: 2024-01-31 16:22:51.207827
...
def upgrade() -> None:
    ...
    op.create_table('handing_fee_config',
    ...
    op.create_table('handing_fee_config_item',
    sa.Column('id', postgresql.UUID(), server_default=sa.text('gen_random_uuid()'), nullable=False, comment='Primary Key'),
    sa.Column('handing_fee_config_id', postgresql.UUID(), nullable=True, comment='Handing Fee Config ID'),
    sa.Column('currency_id', postgresql.UUID(), nullable=True, comment='Currency ID'),
    sa.Column('calculation_type', sa.String(length=32), nullable=True, comment='Calculation Type'),
    ...
    sa.ForeignKeyConstraint(['handing_fee_config_id'], ['public.handing_fee_config.id'], name='handing_fee_config_relation_id_fkey', ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id'),
    ...
    op.add_column('telegram_chat_group', sa.Column('handing_fee_config_id', postgresql.UUID(), nullable=True, comment='Handing Fee Config ID'))
    op.drop_constraint('currency_id_fkey', 'telegram_chat_group', type_='foreignkey')
    op.create_foreign_key('handing_fee_config_relation_id_fkey', 'telegram_chat_group', 'handing_fee_config', ['handing_fee_config_id'], ['id'], source_schema='public', referent_schema='public', ondelete='SET NULL')
...
def downgrade() -> None:
    ...
    op.drop_constraint('handing_fee_config_relation_id_fkey', 'telegram_chat_group', schema='public', type_='foreignkey')
    op.create_foreign_key('currency_id_fkey', 'telegram_chat_group', 'currency', ['currency_id'], ['id'], ondelete='SET NULL')
    op.drop_column('telegram_chat_group', 'handing_fee_config_id')
    ...
    op.drop_table('handing_fee_config_item', schema='public')
    op.drop_table('handing_fee_config', schema='public')
    # ### end Alembic commands ###
